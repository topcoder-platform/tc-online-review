<%--
  ...
  -
  - Description: This page renders the Review Comments for a review item.
  -
  - Version 1.1 (Online Review Update Review Management Process assembly 2) changes:
  - - Display evaluation type for every review item comment.  
--%>
				...
				<td class="value" width="100%">
					<c:choose>
						...
						<c:when test="${comment.commentType.name eq 'Appeal'}">
                            <b>
                            <c:if test="${reviewType ne 'SecondaryReview'}">
                            <bean:message key="editReview.Question.AppealText.title" />:
                            </c:if>
                            <c:if test="${reviewType eq 'SecondaryReview'}">
                            ${orfn:getAppealTitleForEvaluationReview(pageContext, comment)}:
                            </c:if>
                            </b>
                            <c:set var="appealsNum" value="${appealsNum + 1}" />
						</c:when>
						...
						<c:otherwise>
							...
							<c:choose>
								<c:when test="${canPlaceAppealResponse && appealStatuses[itemIdx] eq 'Unresolved' && (comment.commentType.name eq 'Comment' || comment.commentType.name eq 'Required' || comment.commentType.name eq 'Recommended')}">
									...
									</html:select><b id="cmtTypeStatic_${itemIdx}" name="cmtTypeStatic_${itemIdx}" style="display:none;">scuko</b>
								</c:when>
								<c:otherwise>
				...
				<c:choose>
                    <c:when test='${comment.commentType.name == "Comment" || comment.commentType.name == "Required" || comment.commentType.name == "Recommended"}'>
                        <c:choose>
                            ...
                            <c:otherwise>
                            <c:choose>
                            <c:when test="${comment.evaluationType != null}">
                                <c:if
                                    test="${editEvaluation and (appealStatuses[itemIdx] == 'Unresolved')}">
                                    <td class="value" nowrap="nowrap" colspan="${canPlaceAppeal ? 5 : (canPlaceAppealResponse ? 4 : 3)}"><b><bean:message
                                        key="editReview.Evaluation" /></b>
                                        <html:select property="evaluation_type[${itemIdx}]" value="${comment.evaluationType.id}" 
                                        styleClass="inputBox">
                                        <c:forEach items="${allEvaluationTypes}" var="evaluationType">
                                            <html:option value="${evaluationType.id}"
                                                key="EvaluationType.${fn:replace(evaluationType.name, ' ', '')}" />
                                        </c:forEach>
                                    </html:select><span name="cmtEvaTypeStatic_${itemIdx}" style="display:none;">scuko</span></td>
                                </c:if>
                                <c:if
                                    test="${(not editEvaluation) or (appealStatuses[itemIdx] != 'Unresolved')}">
                                    <td class="value" nowrap="nowrap" colspan="${canPlaceAppeal ? 5 : (canPlaceAppealResponse ? 4 : 3)}"><b><bean:message
                                        key="editReview.Evaluation" /></b>
                                        <c:if
                                        test="${not empty comment.evaluationType.name}">
