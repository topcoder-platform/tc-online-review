/**
 * The script contains js variables/functions for edit project links page.
 * This is for "OR Project Linking" Assembly.
 *
 * @author TCSDEVELOPER
 * @version 1.0
 */
...
function newProjectLink() {
  ...
  // This might only happen in tests. if we have enough rows, we will not add it any more
...
function onProjectDropDownChange(projectDropDown) {
	    ...
	    // Sets the value
	    inputBox.value = (projectDropDown.value=='-1')?'':projectDropDown.value;    		    
...
function resetDropDowns () {
  ...
  for(var i=2; i< newLinksTable.rows.length-1; i++) {
      ...
      for(var idx=0; idx < projectOptions.length; idx++) {
      	   ...
      	   // we allow to be here      	         	   
      	   var option = new Option(projectOptions[idx].text,projectOptions[idx].value);
      	   
      	   if(option.value == '-1' || (option.value == selectedValue && i != 2)
      	   ...
      	   if( option.value == selectedValue && i == 2 && valueMap[option.value]) {
      	      inputBox.value = "";
      	   }            	   
      }
...
function resetMsgDiv(inputElement) {
      var msgDiv = getChildByName(inputElement.parentNode, "project_link_validation_msg");
      msgDiv.innerHTML = "";
