...
public class ViewProjectDetailsAction extends BaseProjectDetailsAction {
    ...
    /**
     * This method is an implementation of &quot;View project Details&quot; Struts
     * Action defined for this assembly, which is supposed to gather all possible
     * information about the project and display it to user.
     *
     * @return a string forward to the appropriate page. If no error has occurred,
    ...
    public String execute() throws BaseException {
        ...
        if (!isEmptyOrNull(project.getProperty("Thurgood Platform"))
        ...
        String drpointStr = project.getProperty("DR points") == null ? "0"
                : project.getProperty("DR points").toString();
        if (digitalRunFlag) {
            double drpoint = 0;
            if (drpointStr != null) {
        ...
        boolean isAllowedToViewCockpitProjectName = AuthorizationHelper.hasUserPermission(request,
                Constants.VIEW_COCKPIT_PROJECT_NAME_PERM_NAME);
        ...
        if (myResources.length > 0) {
            ...
            filters.add(
                    LateDeliverableFilterBuilder.createUserHandleFilter((String) myResources[0].getProperty("Handle")));

            List<LateDeliverable> lateDeliverables = lateDeliverableManager
                    .searchAllLateDeliverables(new AndFilter(filters));
        ...
        if (AuthorizationHelper.hasUserPermission(request, Constants.VIEW_LATE_DELIVERABLE_PERM_NAME)) {
            ...
            if (lateDeliverables.size() > 0) {
                ...
                request.setAttribute("unrespondedLateDeliverablesLink", "ViewLateDeliverables?project_id=" + projectId
                        + "&forgiven=Not+forgiven&explanation_status=true&response_status=false");
            }
        }
        ...
        int[] outstandingDeliverableStatuses = getDeliverableStatusCodes(outstandingDeliverables, activePhases,
                currentTime);
        ...
        for (int i = 0; i < phases.length; ++i) {
            ...
            if (scorecardTemplate != null) {
                // override the previous scorecard, here assume the phases are ordered
                // sequentially.
        ...
        // Determine if the user has permission to view a list of resources for the
        // project
        if (AuthorizationHelper.hasUserPermission(request, Constants.VIEW_PROJECT_RESOURCES_PERM_NAME)) {
            ...
            allProjectExtUsers = ActionsHelper.getExternalUsersForResources(ActionsHelper.createUserRetrieval(request),
                    allProjectResources);
        ...
        PhasesDetails phasesDetails = PhasesDetailsServices.getPhasesDetails(request, this, project, phases,
                allProjectResources, allProjectExtUsers);
        ...
        if (AuthorizationHelper.isUserLoggedIn(request)) {
            ...
            Filter filterTNuser = NotificationFilterBuilder
                    .createExternalRefIdFilter(AuthorizationHelper.getLoggedInUserId(request));
        ...
        boolean alreadySubmitted = submitter != null && submitter.getSubmissions() != null
                && submitter.getSubmissions().length > 0;
        ...
        request.setAttribute("isAllowedToManageProjects",
                AuthorizationHelper.hasUserPermission(request, Constants.VIEW_PROJECT_MANAGEMENT_CONSOLE_PERM_NAME));
        ...
        request.setAttribute("isAllowedToEditHisReviews", AuthorizationHelper.hasUserPermission(request,
                Constants.VIEW_REVIEWER_REVIEWS_PERM_NAME)
        ...
        request.setAttribute("isAllowedToPerformSpecReview",
                ActionsHelper.getPhase(phases, true, Constants.SPECIFICATION_REVIEW_PHASE_NAME) != null
                        && AuthorizationHelper.hasUserPermission(request,
                                Constants.PERFORM_SPECIFICATION_REVIEW_PERM_NAME));
        ...
        request.setAttribute("isAllowedToPerformPortMortemReview",
                ActionsHelper.getPhase(phases, true, Constants.POST_MORTEM_PHASE_NAME) != null && AuthorizationHelper
                        .hasUserPermission(request, Constants.PERFORM_POST_MORTEM_REVIEW_PERM_NAME));

        // Checking whether some user is allowed to submit his approval or comments for
        // the
        // Aggregation worksheet needs more robust verification since this check
        // includes a test
        // against whether a user is a submitter, and if it is, whether he is also a
        // winner
        ...
        if (allowedToReviewAggregation && AuthorizationHelper.hasUserRole(request, Constants.SUBMITTER_ROLE_NAME)) {
            ...
            // If current user is a winning submitter, this variable will be reset back to
            // true
            allowedToReviewAggregation = false;

            // Iterate over all 'my' resources looking for 'Submitter' ones and comparing
            // them to the
    ...
    /**
     * <p>
     * Analyzes the current statuses of the specified phases and returns integer
     * codes encoding the current phase statuses. Each
     * </p>
     *
     * @param phases      a <code>Phase</code> array listing the phase to get status
     *                    codes for.
     * @param currentTime a <code>long</code> specifying the current time (in
        ...
        for (int i = 0; i < phases.length; ++i) {
            ...
            if (phaseStatus.equalsIgnoreCase("Scheduled")) {
                ...
                if ((currentTime >= phaseTime) && (ActionsHelper.arePhaseDependenciesMet(phase, true))) {
                    statusCodes[i] = Constants.CANNOT_OPEN_PHASE_STATUS_CODE; // Can't open, phase start time is reached
     ...
     * @param textProvider     the <code>TextProvider</code> instance used to
     *                         retrieve message.
     * @return the hints for the phases which can't be open.
     * @throws PhaseManagementException if error occurs
     */
    private static String[] getPhaseCannotOpenHints(PhaseManager phaseMgr, Phase[] phases, int[] phaseStatusCodes,
            TextProvider textProvider) throws PhaseManagementException {
        // Validate parameters
        ActionsHelper.validateParameterNotNull(phases, "phases");
     ...
     */
    private static int[] getDeliverableStatusCodes(Deliverable[] deliverables, Phase[] activePhases, long currentTime) {
        // Validate parameters
    ...
    /**
     * This static method analyzes the array of deliverables and generates links for
     * them.
     *
     * @return an array of links. If some deliverable does not have its appropriate
     *         link, then that entry in the returned array will be empty (containing
     *         <code>null</code> value).
     * @param request      an <code>HttpServletRequest</code> object.
     * @param deliverables an array of deliverables to generate the links for.
     * @param phases       an array of all phases for the current project.
     * @throws IllegalArgumentException if any of the parameters are
     *                                  <code>null</code>.
     * @throws BaseException            if any error occurs.
     */
    private static String[] generateDeliverableLinks(HttpServletRequest request, Deliverable[] deliverables,
        ...
        for (int i = 0; i < deliverables.length; ++i) {
            ...
            if (delivName.equalsIgnoreCase(Constants.SUBMISSION_DELIVERABLE_NAME)) {
                if (!isStudio) {
                    ...
                    links[i] = "http://" + ApplicationServer.STUDIO_SERVER_NAME + "/?module=ViewContestDetails&ct="
                ...
                Review review = findReviewForSubmission(ActionsHelper.createReviewManager(),
                        LookupHelper.getScorecardType("Specification Review"), deliverable.getSubmission(),
                ...
                Review review = findReviewForSubmission(ActionsHelper.createReviewManager(),
                        LookupHelper.getScorecardType("Screening"), deliverable.getSubmission(),
                        deliverable.getResource(), false);
                ...
                Review review = findReviewForSubmission(ActionsHelper.createReviewManager(),
                        LookupHelper.getScorecardType("Checkpoint Screening"), deliverable.getSubmission(),
                        deliverable.getResource(), false);
                ...
                Review review = findReviewForSubmission(ActionsHelper.createReviewManager(),
                        LookupHelper.getScorecardType("Review"), deliverable.getSubmission(), deliverable.getResource(),
                        false);
                ...
                Review review = findReviewForSubmission(ActionsHelper.createReviewManager(),
                        LookupHelper.getScorecardType("Iterative Review"), deliverable.getSubmission(),
                        deliverable.getResource(), false);
                ...
                Review review = findReviewForSubmission(ActionsHelper.createReviewManager(),
                        LookupHelper.getScorecardType("Checkpoint Review"), deliverable.getSubmission(),
                        deliverable.getResource(), false);
                ...
                Review review = findReviewForSubmission(ActionsHelper.createReviewManager(),
                        LookupHelper.getScorecardType("Review"), deliverable.getSubmission(), aggregator[0].getId(),
                        true);
                ...
                if (myComment == null || !("Approved".equalsIgnoreCase((String) myComment.getExtraInfo())
                ...
                // Skip deliverables with empty Submission ID field, as no links can be
                // generated for such deliverables
                if (deliverable.getSubmission() == null) {
                    continue;
                }

                Review review = findReviewForSubmission(ActionsHelper.createReviewManager(), null,
                ...
                Review[] reviews = ActionsHelper.searchReviews(deliverable.getPhase(), deliverable.getResource(),
                        false);
                ...
                if (reviews.length == 0) {
                    ...
                    // request.setAttribute("postMortemSubmissionId",
     ...
     * @throws BaseException if any error
     */
    private static Long[] getDeliverableSubmissionUserIds(Deliverable[] deliverables) throws BaseException {
    ...
    /**
     * This static method finds and returns a review of specified scorecard template
     * type for specified submission ID and made by specified resource.
     *
     * @return found review or <code>null</code> if no review has been found.
     * @param manager       an instance of <code>ReviewManager</code> class that
     *                      retrieves a review from the database.
     * @param scorecardType a scorecard template type that found review should have
     *                      (null for any)
     * @param submissionId  an ID of the submission which the review was made for.
     ...
     *                      infomration (like all items and their comments).
     * @throws IllegalArgumentException  if <code>submissionId</code> parameter is
     *                                   <code>null</code>, or if
     *                                   <code>submissionId</code> or
     *                                   <code>resourceId</code> parameters contain
     *                                   negative value or zero.
     * @throws ReviewManagementException if any error occurs during review search or
     *                                   retrieval.
     */
    private static Review findReviewForSubmission(ReviewManager manager, ScorecardType scorecardType, Long submissionId,
