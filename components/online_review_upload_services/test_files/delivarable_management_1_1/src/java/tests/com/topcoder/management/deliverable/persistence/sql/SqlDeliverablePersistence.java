...
/**
 * NOTE: THIS IS A MOCK CLASS FOR TEST. <p> The SqlDeliverablePersistence class implements the DeliverablePersistence
 * interface, in order to persist to the database structure given in the deliverable_management.sql script. This class
 * does not cache a Connection to the database. Instead a new Connection is used on every method call.
 * PreparedStatements should be used to execute the SQL statements. </p> <p> This class is immutable and thread-safe in
 * the sense that multiple threads can not corrupt its internal data structures. However, the results if used from
 * multiple threads can be unpredictable as the database is changed from different threads. This can equally well occur
 * when the component is used on multiple machines or multiple instances are used, so this is not a thread-safety
 * concern. </p>
 *
...
public class SqlDeliverablePersistence implements DeliverablePersistence {
    ...
    /**
     * SqlDeliverablePersistence constructor: Creates a new SqlDeliverablePersistence. The connectionName field is set
     * to null.
     *
    ...
    /**
     * SqlDeliverablePersistence constructor: Creates a new SqlDeliverablePersistence. All fields are set to the given
     * values.
     *
     * @param connectionFactory The connection factory to use for getting connections to the database.
    ...
    /**
     * <p> loadDeliverables: Loads the deliverables associated with the given deliverable id. There may be more than one
    ...
    /**
     * <p> loadDeliverable: Loads the deliverable associated with the given submission. The deliverable must be a "per
    ...
    /**
     * <p> loadDeliverables: Loads all Deliverables with the given ids from persistence. May return a 0-length array.
     * </p>
     *
    ...
    /**
     * <p> loadDeliverables: Loads the deliverables associated with the given submissions. The deliverables must be "per
