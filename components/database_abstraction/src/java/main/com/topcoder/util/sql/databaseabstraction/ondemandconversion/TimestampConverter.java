...
/**
 * <p>
 * The TimestampConverter class handles on demand conversions from the Timestamp
 * type to various other data types. It implements the OnDemandConverter
 * interface and has no state.
 * </p>
 * <p>
 * When a value is converted by TimestampConverter, the condition that value can
 * be converted is: value is a Timestamp and not null, and desiredType is Date,
 * Time, Long, or String.
 * </p>
 * <p>
 ...
 * @since 1.1
 */
public class TimestampConverter implements OnDemandConverter {

    /**
     * The format used when converting the Timestamp to a string. This field is
     * set in the constructor and is immutable. It can never be null.
     */
    private final DateFormat dateFormat;

    /**
     * Create a new TimeStampConverter. date format used is initialized to
    ...
    /**
     * Creates a new TimestampCoverter that uses the given format.
     *
     * @param dateFormat
     ...
     * value can be converted is: value is a Timestamp and not null, and
     * desiredType is Date, Time, Long, or String.
     * </p>
     * <p>
     * <strong>Changes in 2.0:</strong>
     * <ol>
    ...
    public boolean canConvert(Object value, int column, CustomResultSetMetaData metaData, Class<?> desiredType) {
        ...
        return AbstractionHelper.canConvert(value, Timestamp.class, desiredType, new Class<?>[] { Time.class,
            Date.class, Long.class, String.class });
    }

    /**
     * <p>
     * Converts value into an instance of desiredType.If value is a Timestamp
     * and not null, and desiredType is Date, Time, Long, or String, value is
    ...
    public Object convert(Object value, int column, CustomResultSetMetaData metaData, Class<?> desiredType)
        ...
        if (desiredType.isAssignableFrom(Date.class)) {
            ...
            return new Time(timestampValue.getTime());
        } else if (desiredType.isAssignableFrom(Long.class)) {
            return new Long(timestampValue.getTime());
        } else { // String
