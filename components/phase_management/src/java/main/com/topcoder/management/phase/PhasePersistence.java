...
/**
 * <p>
 * This is a persistence contract for phase persistence operations. This is broken down into APIs that deal with basic
 * CRUD of phase management (creation, update, deletion) for both individual phases and groups of related phases. The
 * interface also includes methods that allow for retrieval of all phases linked to a particular project or projects.
 * </p>
 * <p>
 * This class works in conjunction with {@link PhaseManager PhaseManager} to insulate the end user from the details of
 * phase persistence.
 * </p>
 * <p>
 * Implementations should provide a constructor that accepts a single <code>String</code> argument representing the
 * name of a {@link com.topcoder.util.config.ConfigManager ConfigManager} namespace. The implementation should
 ...
 * </ul>
 * <p>
 * Thread Safety: Implementations should strive to ensure thread safety for operations. For example, database-centric
 * implementations should utilize database connections local to each method invocation so that there are no issues with
 * multiple threads making simultaneous method calls.
 * </p>
...
public interface PhasePersistence {
     ...
     * @throws IllegalArgumentException if projects is null
     * @throws PhasePersistenceException if an error occurs accessing the persistent store
     */
