 ...
 * @version 1.1
 */
public class DefaultPhaseManagerAccTests extends TestCase {
	/**
	...
	public void testCanStart() throws Exception {
		...
		manager.registerHandler(new MockPhaseHandler(new OperationCheckResult(false, "msg")), type,
			PhaseOperationEnum.START);
		assertFalse("The phase cannot be started.", manager.canStart(phase).isSuccess());
		manager.registerHandler(new MockPhaseHandler(new OperationCheckResult(true, null)), type,
			PhaseOperationEnum.START);
		assertTrue("The phase can be started.", manager.canStart(phase).isSuccess());
	}
	...
	public void testCanEnd() throws Exception {
		...
		assertFalse("The phase cannot be ended.", manager.canEnd(phase).isSuccess());
		manager.registerHandler(new MockPhaseHandler(new OperationCheckResult(true, null)), type,
			PhaseOperationEnum.END);
		assertTrue("The phase can be ended.", manager.canEnd(phase).isSuccess());
	}
	...
	public void testStart() throws Exception {
		...
		assertEquals("Phase status shoud be OPEN", PhaseStatus.OPEN.getId(), phase.getPhaseStatus()
			.getId());
		assertTrue("The actual start date should be updated.", phase.getActualStartDate().after(
			date));
	}
	...
	public void testEnd() throws Exception {
		...
		assertEquals("Phase status shoud be CLOSE", PhaseStatus.CLOSED.getId(), phase
			.getPhaseStatus().getId());
	...
	public void testGetAllHandlers() {
		...
		PhaseHandler handler = new MockPhaseHandler(new OperationCheckResult(false, "msg"));
	...
	public void testGetPhaseValidator() {
		...
		assertTrue("Should return an instance of DefaultPhaseValidator.", manager
			.getPhaseValidator() instanceof DefaultPhaseValidator);
	...
	/**
	 * Tests that updatePhases(Project, String) method.
	 *
	 * @throws Exception
	 *             to JUnit.
	 */
